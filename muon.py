    ...
    def __init__(self, param_groups):
        for group in param_groups:
            ...
            if group["use_muon"]:
                ...
                group["betas"] = group.get("betas", (0.9, 0.95))
                group["eps"] = group.get("eps", 1e-10)
                assert set(group.keys()) == set(["params", "lr", "momentum", "weight_decay", "betas", "eps", "use_muon"])
            else:
                group["lr"] = group.get("lr", 3e-4)
                group["betas"] = group.get("betas", (0.9, 0.95))
                group["eps"] = group.get("eps", 1e-10)
            ...
            if group["use_muon"]:
                ...
                for base_i in range(len(params))[::dist.get_world_size()]:
                    if base_i + dist.get_rank() < len(params):
                        ...
                        # Bước 1 - Nesterov momentum g_t
                        buf = state["momentum_buffer"]
                        buf.lerp_(p.grad, 1 - group["momentum"])
                        g_t = p.grad.lerp(buf, group["momentum"])

                        # Bước 2 - Qua Newton-Schulz sigmoid rect
                        g_t_2d = g_t.view(len(g_t), -1) if g_t.ndim == 4 else g_t.reshape(p.shape)
                        g_t_ns = newton_schulz_sigmoid_rect(g_t_2d)

                        # Bước 3 - adam_update với g_t_ns
                        update = adam_update(g_t_ns.reshape(p.shape),
                                             state["exp_avg"],
                                             state["exp_avg_sq"],
                                             state["step"],
                                             group["betas"],
